        ...
        functions = [
            {
                ...
                "description": "Buscar productos por nombre o descripción. Usar cuando el usuario mencione un producto específico como 'laptop', 'celular', 'ropa', etc.",
                "parameters": {
                    "type": "object",
                    "properties": {
                        "nombre": {
        ...
        try:
            ...
            if response.get("function_call"):
                ...
                try:
                    ...
                    return "Error procesando argumentos de la función."
            ...
            return (
                "Lo siento, no tengo información suficiente para responder tu consulta. "
                "Puedes preguntarme sobre productos, categorías o información general de la tienda. "
                f"Si necesitas ayuda especializada, puedes crear un ticket de soporte y nuestro equipo te ayudará personalmente. "
...
def buscar_productos_por_nombre(nombre: str) -> str:
    ...
    try:
        ...
        if not productos:
            return f"No se encontraron productos que coincidan con '{nombre}'. Si necesitas ayuda específica, puedes crear un ticket de soporte."
        ...
        for i, producto in enumerate(productos, 1):
            ...
            resultado += f"**Descripción:** {producto.descripcion[:100]}{'...' if len(producto.descripcion) > 100 else ''}\n\n"
        
        resultado += "**¿Te interesa alguno de estos productos?** Puedes ver todos los productos disponibles en nuestro catálogo."
...
def buscar_productos_por_categoria(categoria_nombre: str) -> str:
    ...
    try:
        ...
        if not categoria:
            return f"No se encontró la categoría '{categoria_nombre}'. Si necesitas ayuda específica, puedes crear un ticket de soporte."
        ...
        if not productos:
            return f"No hay productos disponibles en la categoría '{categoria.nombre}'. Si necesitas ayuda específica, puedes crear un ticket de soporte."
        
        resultado = f"🏷️ **Categoría: {categoria.nombre}**\n"
        resultado += f"📝 {categoria.descripcion}\n\n"
        resultado += f"**Productos disponibles ({len(productos)}):**\n\n"
        for i, producto in enumerate(productos, 1):
            resultado += f"**{i}. {producto.titulo}**\n"
            resultado += f"💰 **Precio:** Q{producto.precio}\n"
            resultado += f"📦 **Stock:** {producto.stock} unidades\n"
            resultado += f"📝 {producto.descripcion[:100]}{'...' if len(producto.descripcion) > 100 else ''}\n\n"
...
def obtener_detalles_producto(nombre_producto: str) -> str:
    ...
    try:
        ...
        if not producto:
            return f"No se encontró el producto '{nombre_producto}'. Si necesitas ayuda específica, puedes crear un ticket de soporte."
        ...
        resultado += f"🏷️ **Categoría:** {categoria.nombre if categoria else 'N/A'}\n"
        resultado += f"📝 **Descripción completa:**\n{producto.descripcion}\n"
        
        if producto.imagenes and len(producto.imagenes) > 0:
            resultado += f"**Imágenes disponibles:** {len(producto.imagenes)}\n"
        
        resultado += "\n**¿Te interesa este producto?** Puedes ver todos los productos disponibles en nuestro catálogo."
...
def buscar_productos_por_precio(precio_min: float = None, precio_max: float = None) -> str:
    ...
    try:
        ...
        if not productos:
            ...
            return f"No se encontraron productos {rango}. Si necesitas ayuda específica, puedes crear un ticket de soporte."
